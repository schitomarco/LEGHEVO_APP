-- LEGHEVO · correzione notifica uscita autonoma dalla lega
-- Eseguire nel SQL Editor di Supabase dopo 025.
-- Lo script è rieseguibile e non modifica rose, crediti o partecipanti.
...
select
  ...
  coalesce(
    ...
    false
  ) as notification_destination_ready,
  coalesce(
    pg_get_functiondef(
      to_regprocedure('public.leave_league(uuid)')
    ) not ilike '%leagueManagement%',
    false
  ) as invalid_destination_removed,
