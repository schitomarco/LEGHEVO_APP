...
begin
  ...
  if not coalesce((v_application ->> 'protected')::boolean, false)
    ...
    or coalesce(v_release ->> 'activeVersion', '') not in ('0.62.46', '0.62.47', '0.62.48')
    ...
    raise exception 'Preflight v0.62.48 non superato: application=%, release=%, readiness=%.',
...
begin
  loop
    v_claim := public.claim_leghevo_operational_outbox_v2(
      p_environment_key,
      p_destination_key,
      'leghevo-expo-sdk-57-development-client-seed-worker',
      10,
    ...
    loop
      v_application_fingerprint := public.leghevo_sha256_hex_v1(
        p_destination_key || '|expo-sdk-57-development-client-applied|' ||
      ...
      v_outcome := public.apply_leghevo_operational_consumer_message_v1(
        ...
        gen_random_uuid(),
        jsonb_build_object('seedDelivery', true, 'sourceMigration', 156)
      );
      v_count := v_count + 1;
    end loop;
  end loop;
...
begin
  if exists (
    ...
    where certificate.application_version = '0.62.48'
  ) then
    return;
  end if;

  v_outcome := public.certify_leghevo_operational_consumer_v1(
    'production', 'operations_center', 'leghevo-operations-consumer', 11,
    v_operations_consumer_token,
    '62c00000-0000-4000-8000-000000000001'::uuid,
    jsonb_build_object('sourceMigration', 156, 'contract', 'expo-sdk-57-development-client-v1')
  );
  v_outcome := public.certify_leghevo_operational_consumer_v1(
    'production', 'notification_dispatch', 'leghevo-notification-consumer', 11,
    v_notification_consumer_token,
    '62c00000-0000-4000-8000-000000000002'::uuid,
    jsonb_build_object('sourceMigration', 156, 'contract', 'expo-sdk-57-development-client-v1')
  );
  v_outcome := public.certify_leghevo_application_release_v1(
    '0.62.48',
    '23a3abb02f6b6d81eabf5fac135e4632cc7bde559f86bdede3380471ba511d8b',
    '0.62.45',
    '0.62.48',
    '62c00000-0000-4000-8000-000000000003'::uuid,
    jsonb_build_object(
      'baseline', false,
      'sourceMigration', 156,
      'securityPatch', 'expo-sdk-57-development-client-v1'
    )
  );
  v_outcome := public.certify_leghevo_application_rollout_v1(
    'production', '0.62.48', 100, 100, 500, 3, 100,
    '62c00000-0000-4000-8000-000000000004'::uuid,
    jsonb_build_object('strategy', 'expo-sdk-57-development-client-recertification', 'sourceMigration', 156)
  );
  v_outcome := public.activate_leghevo_release_with_rollout_v1(
    'production',
    '0.62.48',
    '62c00000-0000-4000-8000-000000000005'::uuid,
    '62c00000-0000-4000-8000-000000000006'::uuid,
    'release.expo_sdk_57_development_client_activation'
  );
  v_outcome := public.certify_leghevo_operational_telemetry_source_v1(
    'production', 'leghevo-production-observer', 13, v_telemetry_token,
    '62c00000-0000-4000-8000-000000000007'::uuid,
    jsonb_build_object('provider', 'leghevo-runtime', 'sourceMigration', 156)
  );
  v_outcome := public.record_leghevo_authoritative_operational_window_v1(
    'production', 'leghevo-production-observer', 13, v_telemetry_token, 1,
    v_now - interval '5 minutes', v_now, 1000, 1, 0, 175,
    '62c00000-0000-4000-8000-000000000008'::uuid,
  ...
  v_outcome := public.run_leghevo_operational_delivery_audit_v1(
    'production',
    '62c00000-0000-4000-8000-000000000009'::uuid,
    jsonb_build_object('sourceMigration', 156, 'expoSdk57DevelopmentClient', true)
  );
  v_checkpoint := public.create_leghevo_disaster_recovery_checkpoint_v1(
    'production',
    '62c00000-0000-4000-8000-000000000010'::uuid,
    'disaster_recovery.release_0_62_48',
    jsonb_build_object('sourceMigration', 156)
  );
  v_outcome := public.run_leghevo_disaster_recovery_drill_v1(
    'production',
    (v_checkpoint ->> 'checkpointId')::bigint,
    '62c00000-0000-4000-8000-000000000011'::uuid,
    jsonb_build_object('sourceMigration', 156, 'expoSdk57DevelopmentClient', true)
  );
  v_artifact := public.register_leghevo_physical_backup_artifact_v1(
    ...
    v_encryption_hash,
    v_actor_hash,
    '62c00000-0000-4000-8000-000000000012'::uuid,
    jsonb_build_object('evidenceMode', 'external_attestation', 'sourceMigration', 156)
  );
  v_outcome := public.append_leghevo_physical_backup_custody_event_v1(
    ...
    v_storage_hash,
    v_now,
    '62c00000-0000-4000-8000-000000000013'::uuid,
  ...
  v_outcome := public.append_leghevo_physical_backup_custody_event_v1(
    ...
    v_storage_hash,
    v_now,
    '62c00000-0000-4000-8000-000000000014'::uuid,
  ...
  v_outcome := public.run_leghevo_external_restore_rehearsal_v1(
    ...
    v_now,
    v_now,
    '62c00000-0000-4000-8000-000000000015'::uuid,
    jsonb_build_object(
      'isolatedTarget', true,
  ...
  v_run := public.begin_leghevo_service_return_v1(
    'production',
    '62c00000-0000-4000-8000-000000000020'::uuid,
  ...
  v_outcome := public.record_leghevo_service_return_check_v1(
    ...
    v_sequence, v_sequence, '62c00000-0000-4000-8000-000000000021'::uuid,
    jsonb_build_object('sourceMigration', 156)
  );
  ...
  v_outcome := public.record_leghevo_service_return_check_v1(
    ...
    v_sequence, v_sequence, '62c00000-0000-4000-8000-000000000022'::uuid,
    jsonb_build_object('sourceMigration', 156)
  );
  ...
  v_outcome := public.record_leghevo_service_return_check_v1(
    ...
    v_sequence, v_sequence, '62c00000-0000-4000-8000-000000000023'::uuid,
    jsonb_build_object('sourceMigration', 156)
  );
  ...
  v_outcome := public.record_leghevo_service_return_check_v1(
    ...
    v_sequence, v_sequence, '62c00000-0000-4000-8000-000000000024'::uuid,
    jsonb_build_object('sourceMigration', 156)
  );
  ...
  v_outcome := public.record_leghevo_service_return_check_v1(
    ...
    v_sequence, v_sequence, '62c00000-0000-4000-8000-000000000025'::uuid,
    jsonb_build_object('sourceMigration', 156)
  );
  ...
  v_outcome := public.record_leghevo_service_return_check_v1(
    ...
    v_sequence, v_sequence, '62c00000-0000-4000-8000-000000000026'::uuid,
    jsonb_build_object('sourceMigration', 156)
  );
  ...
  v_outcome := public.record_leghevo_service_return_check_v1(
    ...
    v_sequence, v_sequence, '62c00000-0000-4000-8000-000000000027'::uuid,
    jsonb_build_object('sourceMigration', 156)
  );
  ...
  v_outcome := public.record_leghevo_service_return_check_v1(
    ...
    v_sequence, v_sequence, '62c00000-0000-4000-8000-000000000028'::uuid,
    jsonb_build_object('sourceMigration', 156)
  );
  v_outcome := public.complete_leghevo_service_return_v1(
    'production',
    v_run_id,
    '62c00000-0000-4000-8000-000000000030'::uuid,
    'service_return.expo_sdk_57_development_client_certified',
    jsonb_build_object('sourceMigration', 156, 'trafficPercentage', 100)
  );
  v_outcome := public.certify_leghevo_production_readiness_v1(
    'production',
    '62c00000-0000-4000-8000-000000000040'::uuid,
    'production_readiness.expo_sdk_57_development_client_certified',
    jsonb_build_object(
      'sourceMigration', 156,
      'applicationVersion', '0.62.48',
      'checkCount', 10
    )
  );
end;
...
begin
  ...
  if coalesce(v_release ->> 'activeVersion', '') <> '0.62.48'
    ...
    or coalesce(v_readiness ->> 'activeVersion', '') <> '0.62.48' then
    raise exception 'Validazione finale v0.62.48 non superata: release=%, readiness=%.',
